...
# --- 設定 ---
# 檔案名稱
file_gpx = DATA_INTERIM / 'gpx' / 'gpx_analysis_results.csv'
file_html = DATA_INTERIM / 'gpx' / 'webscrapping4HTML_Taoshan_en.csv'
output_file = DATA_INTERIM / 'gpx' / 'merged_taoshan_data.csv'
...
def merge_taoshan_data():
    ...
    try:
        # --- 讀取檔案 (修正點) ---
        # 明確指定使用 'cp950' 編碼來讀取來源檔案  <-- 我跑會報錯 先改utf-8
        print(f"正在嘗試讀取 '{file_gpx}'...")
        df_gpx = pd.read_csv(file_gpx, encoding='utf-8')

        print(f"正在嘗試讀取 '{file_html}'...")
        # 檔案2: 網頁爬取結果 (從檔案內容看是 Tab 分隔的) <-- 我的csv是一般的 拿掉sep='\t'
        df_html = pd.read_csv(file_html, encoding='utf-8')

        print("\n成功讀取檔案。開始進行資料清理與合併...")
        print(f"'{file_gpx}' 原始筆數: {len(df_gpx)}")
        print(f"'{file_html}' 原始筆數: {len(df_html)}")

        print("\ndf_gpx欄位：", df_gpx.columns.tolist())
        print("df_html欄位：", df_html.columns.tolist())
